...
from tensorflow.keras.layers import Conv2D, Convolution2D, MaxPooling2D, Flatten, Dense, Dropout, Activation, BatchNormalization, Input
...
def prepModel( input_shape, bn_layers, dropout_layers, l2_layers, padding, dense_sizes,
    ...
    print ("This error doesn't count from...")
    zzzz = Input(shape=input_shape)
    zzzz = Conv2D(32, (3, 3), padding=padding)(zzzz)
    print ("This error doesn't count to.")

    print("Before Sequential. tf.version={}".format(tensorflow.__version__))
    #model = Sequential()
    ...
    # 2nd CNN
    x = Convolution2D(64, (3,3), padding=padding)(x)
    if "c+2" in bn_layers:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    ...
    # 3rd CNN
    x = Convolution2D(128, (3, 3), padding=padding)(x)
    if "c+3" in bn_layers:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    ...
    # 4th CNN
    x = Convolution2D(256, (3, 3), padding=padding)(x)
    if "c+4" in bn_layers:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    ...
    x = Dense(d_3_size, kernel_regularizer=d_3_regularizer, bias_regularizer=d_3_regularizer)(x)
    if "d-3" in bn_layers:
        x = BatchNormalization()(x)
    ...
    x = Dense(d_2_size, kernel_regularizer=d_2_regularizer, bias_regularizer=d_2_regularizer)(x)
    if "d-2" in bn_layers:
        x = BatchNormalization()(x)
    ...
    x = Dense(d_1_size, activation='softmax')(x)
