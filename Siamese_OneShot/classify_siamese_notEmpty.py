...
dest_folder_notempty = "A:\\IsKnown_Images\\Affine_EmptyNot\\Affine_NotEmpty_Siam\\"
dest_folder_empty = "A:\\IsKnown_Images\\Affine_EmptyNot\\Affine_Empty_Siam\\"
...
for anchor_filename in os.listdir(anchors_folder):
    ...
    cnt_anchors += 1
anchors_all_arr = np.concatenate(anchors_all)
...
def get_distance_from_anchors (img):
    ...
    img_prepped_multiple = np.concatenate([img_prepped for i in range(cnt_anchors)])
    full_input = [anchors_all_arr, img_prepped_multiple]
    pred_empty = model.predict(full_input)
    return np.min(pred_empty), np.average(pred_empty)
...
for img_file in img_file_lst:
    ...
    (min_pred_empty, avg_pred_empty) = get_distance_from_anchors(img_prepped)

    # copy to folder sorted by avg dist
    avg_dist_filename = os.path.join(dest_folder_avg_dist, '{:0>8}.jpg'.format( int(avg_pred_empty * 1e+7) ) )
    #shutil.copy(img_file, avg_dist_filename)

    # copy to folder sorted by min dist
    min_dist_filename = os.path.join(dest_folder_min_dist, '{:0>8}.jpg'.format( int(min_pred_empty * 1e+7) ) )
    #shutil.copy(img_file, min_dist_filename)


    # classes are [0:Empty; 1:NotEmpty]
    dest_folder = dest_folder_notempty if avg_pred_empty <= 0.5 else dest_folder_empty
    ...
    total_copied += 1 if avg_pred_empty <= 0.5 else 0
    loop_cntr +=1

    if loop_cntr%100==0:
        print ("Not empty copied {}, processed {} / {}".format(total_copied,loop_cntr,len(img_file_lst)))
