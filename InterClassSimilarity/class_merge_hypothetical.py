# Class merging based on
#   1) embeddings distance matrix
#   2) confusion matrix biggest error contributors
#   3) som purity improvement
#
...
# Save hypothetical metrics in csv for usage in make_hypoth_merge_graphs.py
hypoth_merge_metrics = np.transpose ( np.vstack ( [
    np.arange(test_conf_mat.shape[0],0,-1),
    hypot_acc_emb_dist,
    hypot_f1_emb_dist,
    hypot_acc_conf_mat,
    hypot_f1_conf_mat,
    hypot_acc_purity_impr,
    hypot_f1_purity_impr
]))
df_hypoth_merge_metrics = pd.DataFrame( hypoth_merge_metrics,
                                        columns = ['class_cnt',
                                                   'hypot_acc_emb_dist','hypot_f1_emb_dist',
                                                   'hypot_acc_conf_mat','hypot_f1_conf_mat',
                                                   'hypot_acc_purity_impr','hypot_f1_purity_impr'])
df_hypoth_merge_metrics.to_csv("metrics_hypoth_merge.csv", header=True, index=None)
