# Trains (overfits) NN
#   Train data: mostly 1 class (empty)
#   No earlystopping or
#   No restore best weights
#   No save best weights
...
def trainModel(epochs=1, isresnet=False):
    ...
    mcp_save = ModelCheckpoint(model_file_name, save_best_only=False, monitor='val_acc', mode='max')    #save_best_only=False - save last
    ...
    model.fit(train_iterator, steps_per_epoch=len(train_iterator),
              ...
              callbacks=[  callback_csv_logger])    #mcp_save, callback_earlystop,
...
trainModel(epochs=200, isresnet=False)