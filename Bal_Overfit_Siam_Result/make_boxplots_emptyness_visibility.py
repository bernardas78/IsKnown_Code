...
yticks = np.arange(0.64, 0.76, 0.02)
plt.yticks(ticks=yticks, labels=[ "{:.2f}".format(ytick) for ytick in yticks])

plt.xticks(ticks=(np.arange(len(lst_x_labels)))+1, labels=lst_x_labels, rotation=90)
plt.title ("Test accuracy by visibility", fontdict={'fontname':'calibri', 'fontsize':24})
legend = plt.legend(["Q1: <1/4 product area visibility", "BagR: plastic bags with high glare"], loc='lower left', handlelength=0, fontsize=15)
...
plt.yticks(ticks=yticks, labels=[ "{:.2f}".format(ytick) for ytick in yticks])
plt.xticks(ticks=(np.arange(len(lst_x_labels)))+1, labels=lst_x_labels, rotation=90)
plt.title ("Test accuracy by emptyness classifier", fontdict={'fontname':'calibri', 'fontsize':24})
